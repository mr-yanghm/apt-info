...
const AptInfo = ({
  ...
  return (
    <>
      <article>
        <div>
          <h1>{myApt.kaptName} 아파트 최근 거래</h1>
      ...
      <article>
              ...
              nearAptPriceInfo.map((apt, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td className="center">
                      {to_date_format(apt.거래년월)}
                      <button
                        className="habit-button habit-delete"
                        onClick={onDelete.bind(this, apt)}
                      >
        ...
        <div className="addFilter">
          <button type="button" id="addFilter" onClick={onClickAddFilter}>
            {isVisibleSearchbox ? "필터 숨기기" : "필터 보이기"}
          </button>
        </div>
      </article>
    </>
